...
from data_loader import ECGDataset, FocalLoss, get_snomed_list
...
def convert_features_to_tensor(features):
    ...
    for feature_dict in features:
        ...
        array_features = []
        #print("DAJEEEE ROMA: ", feature_dict.size())
        for key, value in feature_dict.items():
            if isinstance(value, np.ndarray):
                ...
                #array_features.append(torch.tensor(value).float())
            # array_features = [array_features.appendtorch.tensor(value).float() for key, value in feature_dict.items()
            #               if isinstance(value, np.ndarray)]
            #print("ARRAY DIOCANE ", torch.tensor(value).float().shape)
...
def collate_fn(batch, dataset):
    ...
    # features_paths = [item[3] for item in batch]

    # data_tensors = [d for d in data]
    # features = []
    # for path in features_paths:
    #     with np.load(path, allow_pickle=True) as data:
    #         features.append(dict(data))
    # features_tensor = convert_features_to_tensor(features)
...
def train_model(num_workers = 0):
    ...
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=0, collate_fn=lambda batch: collate_fn(batch, dataset), pin_memory=True, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False, num_workers=0, collate_fn=lambda batch: collate_fn(batch, dataset), pin_memory=True, drop_last=True)
    test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False, num_workers=0, collate_fn=lambda batch: collate_fn(batch, dataset), pin_memory=True, drop_last=True)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (data, labels, masks, features) in enumerate(train_loader):
            #print("110")
            data = data.to(device)
            ...
            #print("AAOSKFLASFKAL ", len(labels))

        #print("LUNGHEZZA MINCHIA ", len(train_dataset), running_loss, len(train_loader))
        ...
        with torch.no_grad():
            ...
            for data, labels, masks, features in val_loader:
                #print("INSIDE")
                #print(data, labels, masks, features)
                data = data.to(device)
                labels = labels.to(device)
                masks = masks.to(device)
                features = features.to(device)
